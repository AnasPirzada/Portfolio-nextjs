...
const GitHubContributions = ({ username = 'AnasPirzada' }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full bg-gradient-to-br from-gray-900/50 to-black/50 rounded-2xl p-6 border border-white/10 hover:border-[#eeba2c]/30 transition-all"
    >
        ...
        <>
          <div className="mb-4 flex gap-4 text-sm">
            <div className="text-gray-400">
              <span className="text-[#eeba2c] font-semibold">{contributions.total}</span> contributions last year
            </div>
            {contributions.streak && (
              <div className="text-gray-400">
                <span className="text-[#eeba2c] font-semibold">{contributions.streak}</span> day streak
              </div>
            )}
          </div>
